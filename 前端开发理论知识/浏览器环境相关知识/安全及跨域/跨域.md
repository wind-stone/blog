...
AJAX 同源策略主要用来防止CSRF攻击。如果没有 AJAX 同源策略，相当危险，我们发起的每一次 HTTP 请求都会带上请求地址对应的 cookie，那么可以做如下攻击：
...
优点：CORS支持所有类型的HTTP请求，是跨域 HTTP 请求的根本解决方案
...
一般在项目开发阶段，会使用本地服务器转发接口（比如可以通过webpack 的`webpack-dev-server`去配置`proxy`），从而达到通过服务器代理解决跨域问题。
