...
- 根元素盒子的外边距不会折叠
- 若具有[安全间距](/css-spec/css2.2/9-visual-formatting-model/#_9-5-2-控制浮动旁边的流-clear-属性)的元素的上外边距和下外边距毗邻，则元素的外边距将与其随后的兄弟元素的毗邻的外边距折叠，但是最后产生的外边距不会与父块的下外边距折叠。（TODO: WHY?）
