 ...
 *
 * Example:
 *   var str = glEnumToString(ctx.getError());
 *
...
function glFunctionArgToString(
    functionName: keyof typeof glValidEnumContexts,
...
type propertyNames = keyof typeof WebGLRenderingContext;
function init(ctx: WebGLRenderingContext) {
    if (glEnums == null) {
        glEnums = {};
        for (const propertyName in ctx) {
            if (typeof ctx[propertyName] == 'number') {
                // console.log('+++ init propertyName', propertyName, ctx[propertyName]);
            ...
            for (var ii = 0; ii < args.length; ++ii) {
                argStr +=
                    ...
                    glFunctionArgToString(
                        functionName as keyof typeof glValidEnumContexts,
                        ii,
                        args[ii]
                    );
            }
